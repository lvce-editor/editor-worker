...
test.each(['return `file:///${encodeFilePath(normalizedPath)}`', 'return `file://${encodeFilePath(path)}`', 'const url = `https://${host}/path`'])(
  'does not detect links in dynamic JavaScript template strings',
  (text) => {
