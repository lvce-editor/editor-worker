...
export const name = 'editor.diagnostics-small-scroll-preserves-scroll'

export const test: Test = async ({ Command, Editor, expect, Extension, FileSystem, Locator, Main, Settings, Workspace }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const uri = `${tmpDir}/small-scroll-preserves-scroll.diagnostics-scroll`
  ...
  const expectedRow = Locator('.EditorRow', { hasText: 'é line 11' })
