...
export const name = 'editor.title-bar-selection-shrink-selection'
...
export const test: Test = async ({ ContextMenu, Editor, FileSystem, Main, TitleBarMenuBar, Workspace }) => {
  ...
  await Editor.setSelections(new Uint32Array([0, 0, 0, 14]))

  await selectTitleBarSelectionItem(TitleBarMenuBar, ContextMenu, 'Shrink Selection')
