...
export const name = 'editor.title-bar-selection-select-all-occurrences'
...
export const test: Test = async ({ ContextMenu, Editor, FileSystem, Main, TitleBarMenuBar, Workspace }) => {
  ...
  await selectTitleBarSelectionItem(TitleBarMenuBar, ContextMenu, 'Select All Occurrences')

  await Editor.shouldHaveSelections(new Uint32Array([0, 0, 0, 3, 0, 4, 0, 7, 0, 8, 0, 11]))
}
