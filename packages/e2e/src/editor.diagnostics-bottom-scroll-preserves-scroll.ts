...
export const name = 'editor.diagnostics-bottom-scroll-preserves-scroll'

export const test: Test = async ({ Command, Editor, expect, Extension, FileSystem, Locator, Main, Settings, Workspace }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const uri = `${tmpDir}/bottom-scroll-preserves-scroll.diagnostics-scroll`
  ...
  const expectedRow = Locator('.EditorRow', { hasText: 'é line 195' })
