...
export const name = 'editor.diagnostics-already-scrolled-preserves-latest-scroll'

export const test: Test = async ({ Command, Editor, expect, Extension, FileSystem, Locator, Main, Settings, Workspace }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const uri = `${tmpDir}/already-scrolled-preserves-latest-scroll.diagnostics-scroll`
  ...
  await Editor.setDeltaY(400)
  const initialExpectedRow = Locator('.EditorRow', { hasText: 'é line 21' })
  await expect(initialExpectedRow).toBeVisible()

  await Command.executeExtensionCommand('diagnosticsScroll.resolveMany', 1, 120)
  await new Promise((resolve) => setTimeout(resolve, 2))
  await Editor.setDeltaY(1200)

  const diagnosticMarkers = Locator('.Editor .ScrollBarDiagnostic')
  for (let attempt = 0; attempt < 200; attempt++) {
    try {
      await expect(diagnosticMarkers).toHaveCount(120)
      break
    } catch (error) {
      if (attempt === 199) {
        throw error
  ...
  const expectedRow = Locator('.EditorRow', { hasText: 'é line 81' })
