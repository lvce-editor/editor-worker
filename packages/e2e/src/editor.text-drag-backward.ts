...
export const name = 'editor.text-drag-backward'

export const test: Test = async ({ Command, Editor, FileSystem, Main, Workspace }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const filePath = `${tmpDir}/text-drag-backward.txt`
  ...
  await Editor.shouldHaveText('betaalpha  gamma')
  await Editor.shouldHaveSelections(new Uint32Array([0, 0, 0, 4]))
}
